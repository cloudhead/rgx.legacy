...
impl Constraints {
    ...
    pub fn new(min: Size, max: Size) -> Constraints {
        Constraints {
            min: min.expand(),
            max: max.expand(),
        }
    ...
    ///
    /// [rounded away from zero]: struct.Size.html#method.expand
    pub fn shrink(&self, diff: impl Into<Size>) -> Constraints {
        let diff = diff.into().expand();
        let min = Size::new((self.min.w - diff.w).max(0.), (self.min.h - diff.w).max(0.));
        let max = Size::new((self.max.w - diff.w).max(0.), (self.max.h - diff.w).max(0.));
    ...
    pub fn contains(&self, size: impl Into<Size>) -> bool {
        let size = size.into();
        (self.min.w <= size.w && size.w <= self.max.w)
            && (self.min.h <= size.w && size.w <= self.max.h)
    }
    ...
    /// `Size` will be chosen, and likewise if `width == f32::INFINITY`, then the largest `Size`
    /// will be chosen.
    ///
    ...
    pub fn constrain_aspect_ratio(&self, aspect_ratio: f32, width: f32) -> Size {
        ...
        let min_w_min_h = self.min.h / self.min.w;
        let max_w_min_h = self.min.h / self.max.w;
        let min_w_max_h = self.max.h / self.min.w;
        let max_w_max_h = self.max.h / self.max.w;
        ...
        if aspect_ratio > min_w_max_h {
            ...
            if width < self.min.w {
                ...
                Size {
                    w: self.min.w,
                    h: self.min.w * aspect_ratio,
                }
            } else if aspect_ratio < max_w_max_h {
                // exits through max.width
                Size {
                    w: self.max.w,
                    h: self.max.w * aspect_ratio,
                }
            } else {
                // exits through max.height
                Size {
                    w: self.max.h * aspect_ratio.recip(),
                    h: self.max.h,
                }
            }
        } else {
            ...
            if width < self.min.w {
                ...
                Size {
                    w: self.min.h * aspect_ratio.recip(),
                    h: self.min.h,
                }
            } else if aspect_ratio > max_w_max_h {
...
mod tests {
    ...
    fn bc(min_width: f32, min_height: f32, max_width: f32, max_height: f32) -> Constraints {
        Constraints::new(
    ...
    #[test]
    fn unbounded() {
        assert!(!Constraints::UNBOUNDED.is_width_bounded());
        assert!(!Constraints::UNBOUNDED.is_height_bounded());

        assert_eq!(Constraints::UNBOUNDED.min(), Size::ZERO);
    }
}
