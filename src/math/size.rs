...
use super::{traits::FloatExt as _, Vector2D, Zero};
...
impl Size<f32> {
    /// Returns a new `Size`, with `width` and `height` rounded away from zero to the nearest
    /// integer, unless they are already an integer.
    ///
    /// # Examples
    ///
    /// ```
